...
var PROJECT_ID = `pi-ostelco-${ENVIRONMENT}`;
...
function getAccessToken() {
  return new Promise(function(resolve, reject) {
    var key = require(`./.secrets/pi-ostelco-${ENVIRONMENT}-firebase-adminsdk.json`);
...
function publishTemplate(etag) {
  getAccessToken().then(function(accessToken) {
    var options = {
      method: 'PUT',
      uri: HOST + PATH,
      body: fs.readFileSync(`remote-config-${ENVIRONMENT}.json`, 'UTF8'),
      gzip: true,
      resolveWithFullResponse: true,
      headers: {
        'Authorization': 'Bearer ' + accessToken,
